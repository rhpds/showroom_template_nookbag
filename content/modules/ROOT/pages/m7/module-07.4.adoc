...
. Uncomment the `--- ORCHESTRATOR FEATURE ---` section in the YAML, to enable the Orchestrator.
+
image:m7/orchestrator-enabled-flag.png[Orchestrator Enabled Flag, width=60%]
. Next find the `--- ORCHESTRATOR ADDITIONAL PLUGINs ---` section, and uncomment the lines within to enable the notifications plugin(s) that are used in the workflow in this section.
+
image:m7/orchestrator-plugin.png[Orchestrator and Notification dynamic plugins, width=60%]
+
. Look for the YAML between the `--- ORCHESTRATOR ---` block further down in the file and uncomment it.
+
...
. Select the image:common/plus-icon.png[width=15px] icon on the top navigation bar to see the new templates.
+
...
. Verify in the https://console-openshift-console.{openshift_cluster_ingress_domain}/k8s/ns/backstage/core%7Ev1%7EPod[Backstage namespace in OpenShift^, window="console"] that a pod is created for the Serverless Workflow instance. The pod name starts with `create-ocp-namespace-swt`.
+
...
.. *Requester*: `dev1`
.. *Size*: `small`
.. *Reason*: type any message you like here
