...
The template gives you a clean structure with zero boilerplate, the Red Hat Demo Platform UI theme, and a dev-mode extension for previewing variables.
...
<2> JavaScript extensions (dev-mode, attribute injection, etc.).
<3> Custom CSS or HTML partials that override the UI theme.
<4> Navigation file -- controls the sidebar table of contents.
<5> Images referenced from your AsciiDoc pages.
<6> Downloadable files (scripts, configs) that readers can copy.
<7> Your lab pages -- one AsciiDoc file per module or section.
<8> Reusable AsciiDoc snippets you can include inline.
<9> Production Antora playbook -- uses the Red Hat theme, no dev-mode.
<10> Development Antora playbook -- enables dev-mode and uses a simpler theme.
<11> Showroom UI configuration -- defines tabs, layout width, and dynamic URLs.
...
Your content repo needs an Antora playbook file -- typically `site.yml` (or `default-site.yml`).
...
When running Antora locally, you can enable dev-mode by adding the extension to your playbook.
The template's `default-site.yml` has it enabled by default:
...
Add the extension to your `site.yml` or `default-site.yml` playbook:
