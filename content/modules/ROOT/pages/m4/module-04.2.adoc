...
+
NOTE: This YAML snippet enables auto-discovery for all files named _all-location.yaml_ (the `entityFilename` option) where the repo name starts with the word _all-location_ (the `projectPattern` option). 
